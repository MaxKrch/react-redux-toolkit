...
const TaskItem = ({
    ...
    const sendChange = () => {
        ...
        onChange(
            task.id,
            descriptionElement.current.textContent.trim()
        ) 
    }
    ...
    const secondButtonConfig = isEditing 
        ? ({ title: 'Cancel', onClick: () => {
            if(descriptionElement.current) descriptionElement.current.textContent = task.description
            onCancel(task.id)
        }})
        : ({ title: 'Delete', onClick: () => onRemove(task.id)})

    const buttonClasses = 'px-3 py-1 uppercase cursor-pointer font-semibold min-w-32'
